# -*- coding: utf-8 -*-
"""
Created on Thu Aug 23 10:06:46 2018
...
def discuss(max_evals):
    ...
    for t in topicOfPlaylist:
        ...
        if(len(needTest)==0 ):#or TrackNum<500 or len(currentPid)>200 ):
            ...
            print(best['pw'],best['aw'],best['tw'],"单个主题下的平均准确率",MAP,mndcg)
            allAccuracy=pd.concat([allAccuracy,bestAccuracys],axis=0,ignore_index=True)#记录所有待验证的pid,及准确率,ndcg
            topic_detail.append([t,best['pw'],best['aw'],best['tw'],MAP,mndcg,len(needTest),len(currentPid)])
    s=pd.DataFrame(topic_detail,columns=['topic','pw','aw','tw','meanRecall','meanNDCG@100','needTestNum','hasPidNum'])
    s.to_csv("NeedConcern/topic_detail",index_label=False)
    allAccuracy.to_csv("NeedConcern/allAccuracy_detail_withNDCG",index_label=False)
    print("Done!")                
    return
discuss(30)