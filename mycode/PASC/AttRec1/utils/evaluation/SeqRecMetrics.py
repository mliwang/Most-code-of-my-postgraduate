...
def evaluate_ranking(self, test, train=None, k=10):
    test = test.tocsr()
    ...
    for _k in ks:  #_k即要为用户推荐的url数目
          ...
          for user_id, row in enumerate(test):
            ...
            predictions = -self.predict2(user_id)
            predictions = predictions.argsort()

            if len(predictions) > _k:
                predictions = predictions[:_k]

            targets = row.indices
            n2 = n2 + len(targets)
            extrat_ask = len(set(targets) - set(predictions))
            n1 = n1 + extrat_ask
            C.append((_k - len(set(targets).intersection(set(predictions)))
                      ) / _k)
          n=n1/n2
          c=sum(C) / len(C)
          NetworkLoadRate.append(n)
          CacheRaplaceRate.append(c)
...
def evaluate1(self):
    ...
    #获取用户隐层向量、自注意力层输出、用户特定的偏置、注意力权值：getUserParam()
    ...
    #根据公式y=w(U-V_i)^2+(1-w)[m-X_u(t+1)]^2 计算预测结果
