...
class Recommender(object):
    ...
    def _initialize(self, interactions):
        ...
        self._net = AttRec(self.sess, self._num_users,  self._num_items, l2 = self.args.l2)

        #构建模型，输入马尔科夫链的阶数，和目标的长度
        self._net.build_network(L = self.args.L, num_T=self.args.T, num_factor=self.args.d, num_neg = self._neg_samples)
        
        init = tf.global_variables_initializer()
        self.sess.run(init) 
        

    def fit(self, train, test, val, verbose=False):
        """
        ...
        users_np = train.sequences.user_ids.reshape(-1,)
        ...
        for epoch_num in range(start_epoch, self._n_iter):
            ...
            for (minibatch_num, (batch_users, batch_sequences, batch_targets, batch_negatives)) in enumerate(minibatch(users_np, sequences_np, targets_np, negatives_np, batch_size=self._batch_size)):
                ...
                loss = self._net.train2(self.sess, batch_users, batch_sequences, batch_targets, batch_negatives)
        ...
        #最终测试集评价：
        NetworkLoadRate, CacheRaplaceRate = evaluate_ranking(self, val, k=[100,200,300,400,500,600,700,800,900,1000])
        print("NetworkLoadRate:",NetworkLoadRate)
        print("CacheRaplaceRate:", CacheRaplaceRate)
    ...
    def predict(self, user_id, item_ids=None):
        ...
        # print("predict:",out)
        out = self._net.predict2(self.sess, user_id, sequences_np, item_ids)
...
def main():
    ...
    parser.add_argument('--val_root', type=str, default='data/ml100k/temp/test.dat')#data/ml100k/temp/test.dat
    parser.add_argument('--L', type=int, default=3) # length of sequence
    parser.add_argument('--T', type=int, default=1)# number of targets
    # train arguments
    parser.add_argument('--n_iter', type=int, default=50)#150
    parser.add_argument('--seed', type=int, default=1234)
    parser.add_argument('--batch_size', type=int, default=128)
    parser.add_argument('--learning_rate', type=float, default=1e-3)
    parser.add_argument('--l2', type=float, default=0.001)#1e-6
    parser.add_argument('--neg_samples', type=int, default=3)  #
    # model arguments
    parser.add_argument('--d', type=int, default=150)#50
    ...
    val = Interactions(config.val_root,
                        user_map=test.user_map,
                        item_map=test.item_map)

    print(config)
    # fit model

    model = Recommender(args=config)
    model.fit(train, test, val, verbose=True)
