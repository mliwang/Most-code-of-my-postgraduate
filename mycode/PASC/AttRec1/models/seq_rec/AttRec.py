...
class AttRec():#reg_rate=0.1
    def __init__(self, sess, num_user, num_item, l2, learning_rate=0.05, reg_rate=1e-2, epoch=5000, batch_size=1000,
                 show_time=False, T=1, display_step=256, verbose=False):#display_step =1000 
    ...
    def build_network(self, L, num_T, num_factor=100, num_neg=1):#300
        ...
        #self.user_id = tf.reshape(self.user_id, [128])
        ...
        #print("self.user_id.shape:", np.shape(self.user_id))
        ...
        #损失计算：loss = Σ[y(-) + γ - y(+)] + λΘ, 其中Θ ={X,V,U,W_Q,W_k}
        """
        ...
        #计算损失
        y=tf.nn.sigmoid(self.target_prediction)
        positive_loss = -tf.reduce_mean(tf.log(tf.clip_by_value(y, 1e-8, tf.reduce_max(y))))
        y1=1 - tf.nn.sigmoid(self.negative_prediction)
        negative_loss = -tf.reduce_mean(tf.log( tf.clip_by_value(y1, 1e-8, tf.reduce_max(y1))))
        l2_loss = self.l2 * (tf.nn.l2_loss(self.P) + tf.nn.l2_loss(self.V) + tf.nn.l2_loss(self.X) + tf.nn.l2_loss(self.Q))
    ...
    def getUserParam(self, user_id):
        #获取用户隐层向量、自注意力层输出、用户特定的偏置、注意力权值
    ...
    def _distance_self_attention(self, item_seq, user_id, item_id, isTrain=True):
        ...
        #为权值加上掩码，掩盖掉权值的对角线值
        #tf.matrix_set_diag将输入矩阵的对角元素置换为对角元素。
        ...
        #对L阶的注意力加权输出求平均（1/L*out）
        #print("item_latent_factor.shape:", np.shape(item_latent_factor))
        self.out = tf.reduce_mean(out, 1)
        #print("out.shape:", np.shape(self.out))

        #得到物品序列：w_items = V_i
        w_items =  tf.nn.embedding_lookup(self.X, item_id)
        #得到原始物品序列：w_items_2 = X_u(t+1)
        w_items_2 = tf.nn.embedding_lookup(self.Q, item_id)
        ...
        #print("w_items.shape:", np.shape(w_items))
        #转置
        ...
        self.alpha = 0.2
        if isTrain:
            #最终预测目标：y=w(U-V_i)^2+(1-w)[m-X_u(t+1)]^2
    ...
    def prepare_data(self, train_data, test_data):
        ...
        #将test数据整形成user_id 对应row_index的形式
    ...
    def train2(self, sess, batch_users, batch_sequences, batch_targets, batch_negatives):
        """
        Parameters
        ----------
        ...
        item_var: torch.LongTensor with size [batch_size]
            a batch of items
        """
        _, loss = sess.run((self.optimizer, self.loss), feed_dict={self.user_id: batch_users,
                                                                            self.item_seq: batch_sequences,
                                                                            self.item_id: batch_targets,
                                                                            self.neg_item_id: batch_negatives,
                                                                            self.isTrain: True})
        return loss
    ...
    def test(self, test_data):
        ...
        #执行评价：evaluate1()
        #evaluate1(self)
        NetworkLoadRate, CacheRaplaceRate = evaluate_ranking(self, test_data, k=[100,200,300,400,500,600,700,800,900,1000])
    ...
    def predict2(self, sess, user_id, sequences_np, item_ids):
        """
        ...
        return sess.run([self.test_prediction], feed_dict={self.user_id: user_id,
                                                                  self.item_seq: sequences_np,
                                                                  self.item_id_test: item_ids})[0]
