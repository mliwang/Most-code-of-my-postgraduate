...
def evaluate_ranking(model, test, train=None, k=10):
    ...
    for _k in ks:  #_k即要为用户推荐的url数目
          ...
          for user_id, row in enumerate(test):
            ...
            predictions = -model.predict(user_id) #train_caser.py中的predict 为啥要取负号？？？
            # predictions1=model.predict(user_id)
            ...
            n1 = n1 + extrat_ask
            C.append((len(set(targets).intersection(set(predictions)))
                      ))
          n=n1/n2
          c=sum(C) / len(C)
          # print(_k, " ", n1 / n2, " ", sum(C) / len(C))
